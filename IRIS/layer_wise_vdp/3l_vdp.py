...
def find_top_train(max_loss=83):
    ...
    torch.save(model.state_dict(), 'loo_params_3l.pt')
...
def main(n_iters):
    ...
    while i < n_iters:
        ...
        if i % 5 == 0:
            np.save('figure1/vdp_3l_train.npy', train, allow_pickle=True)
            np.save('figure1/vdp_3l_eig.npy', eig, allow_pickle=True)
            np.save('figure1/vdp_3l_pearson.npy', pearson, allow_pickle=True)
            np.save('figure1/vdp_3l_spearman.npy', spearman, allow_pickle=True)
        i += 1
    np.save('figure1/vdp_3l_train.npy', train, allow_pickle=True)
    np.save('figure1/vdp_3l_eig.npy', eig, allow_pickle=True)
    np.save('figure1/vdp_3l_pearson.npy', pearson, allow_pickle=True)
