...
def find_top_train(max_loss=83):
    ...
    torch.save(model.state_dict(), 'loo_params_4l.pt')
...
def exact_difference(model, top_train, max_loss):
    ...
    for i in top_train:
        ...
        # model = Model(x.shape[1], 5, 3).to('cuda:0')
        model.load_state_dict(torch.load('loo_params_4l.pt'))
...
def main(n_iters):
    ...
    while i < n_iters:
        ...
        if i % 5 == 0:
            np.save('figure1/vdp_4l_train.npy', train, allow_pickle=True)
            np.save('figure1/vdp_4l_eig.npy', eig, allow_pickle=True)
            np.save('figure1/vdp_4l_pearson.npy', pearson, allow_pickle=True)
            np.save('figure1/vdp_4l_spearman.npy', spearman, allow_pickle=True)
        i += 1
    np.save('figure1/vdp_4l_train.npy', train, allow_pickle=True)
    np.save('figure1/vdp_4l_eig.npy', eig, allow_pickle=True)
    np.save('figure1/vdp_4l_pearson.npy', pearson, allow_pickle=True)
