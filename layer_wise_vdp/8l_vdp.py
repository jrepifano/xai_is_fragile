...
def main(n_iters):
    ...
    while i < n_iters:
        ...
        if i % 5 == 0:
            np.save('figure1/vdp_8l_train.npy', train, allow_pickle=True)
            np.save('figure1/vdp_8l_eig.npy', eig, allow_pickle=True)
            np.save('figure1/vdp_8l_pearson.npy', pearson, allow_pickle=True)
            np.save('figure1/vdp_8l_spearman.npy', spearman, allow_pickle=True)
        i += 1
    np.save('figure1/vdp_8l_train.npy', train, allow_pickle=True)
    np.save('figure1/vdp_8l_eig.npy', eig, allow_pickle=True)
    np.save('figure1/vdp_8l_pearson.npy', pearson, allow_pickle=True)
