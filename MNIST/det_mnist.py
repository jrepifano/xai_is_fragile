...
class Model(torch.nn.Module):
    ...
    def score(self, x, y):
        ...
        if x.device.type != device or y.device.type != device:
            x, y = x.to(device), y.to(device)
        dataloader = DataLoader(data_loader(x, y), 10000, shuffle=False, num_workers=2, pin_memory=True)
        logits = list()
        for idx, (image, target) in enumerate(dataloader):
            logits.append(torch.nn.functional.softmax(self.forward(image), dim=1).detach().cpu().numpy())
        score = np.sum(np.argmax(np.vstack(logits), dim=1) == y.cpu().numpy())/len(target)
        return score
