...
class lenet(pl.LightningModule):
    def __init__(self, batch_size, train_idx_to_remove=None, test_idx=None, fine_tune=False):
        super().__init__()
    ...
    def configure_optimizers(self):
        if self.fine_tune:
            optimizer = torch.optim.Adam(self.parameters(), lr=1e-8, weight_decay=0.001)
        else:
    ...
    def get_losses(self, set='train'):
        ...
        dl = DataLoader(mnist, batch_size=self.batch_size, num_workers=2, shuffle=False, pin_memory=True)
        if self.test_idx != None:
...
def finetune(gpu, top_40, test_idx, true_loss, batch_size):
    ...
    for counter, idx in enumerate(top_40):
        model = lenet(batch_size=batch_size, train_idx_to_remove=idx, test_idx=test_idx, fine_tune=True)
        model.load_state_dict(torch.load('lenet.pt'))
        ...
        for idx, (x, y) in enumerate(model.test_dataloader()):
            model.x_test = x.to('cuda')
            model.y_test = y.to('cuda')
        no_epochs = 10
